    ...
    async renders() {        
        ...
        try{    
            ...
            if(cityData && cityData.data && cityData.data.cityId){            
                ...
                // 跳转到上海
                this.res.redirect('/shanghai/xflist/' + (channel&&("?channel=" +channel)||""));
                return;
            }
            /*++-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if( this.req.cookies.visitedCityId && this.req.cookies.visitedCityId != cityModel.cityId && this.req.cookies.selectedCityId && this.req.cookies.selectedCityId != cityModel.cityId ) {
                ...
                this.res.redirect("/" + this.req.cookies.visitedCityPinyin + "/xflist" + (channel&&("?channel=" + channel)||"")) ;
                return ;
            }
            /*++-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            坑爹适配结束
            ...
            Object.assign(this.templateData, {                 
                ...
                "data": data,
                "channel": channel ,
                "routerConditionParams" : this.req.params.condition || "" ,  //广告在无路由condition参数的情况下才显示
